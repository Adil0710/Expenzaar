...
export const useExpensesStore = create<ExpensesState>((set, get) => ({
  ...
  setDateRange: (startDate, endDate) => {
    ...
    const totalPages = Math.ceil(filteredExpenses.length / get().pagination.pageSize)
  ...
  clearFilters: () => {
    ...
    const totalPages = Math.ceil(allExpenses.length / get().pagination.pageSize)
  ...
  getFilteredExpenses: () => {
    ...
    return allExpenses.filter((expense) => {
      ...
      if (filters.categoryId) {
        ...
        if (!expense.category || String(expense.category.id) !== String(filters.categoryId)) {
          return false
        }
      }
      ...
      if (filters.searchTerm) {
        ...
        const matchesCategoryName = (expense.category?.name || "").toLowerCase().includes(searchTerm)
  ...
  deleteExpense: async (expenseId) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const updatedAllExpenses = get().allExpenses.filter((expense) => expense.id !== expenseId)
